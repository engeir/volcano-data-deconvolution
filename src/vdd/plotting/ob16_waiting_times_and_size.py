...
so2_peaks_big_ser = so2_peaks_big.to_series()
so2_peaks_small = so2_peaks.where(so2_peaks < SMALL_LIMIT).dropna("time")
so2_peaks_small_ser = so2_peaks_small.to_series()
isolated_normal = []
i = 0
...
peaks_normal_ser = peaks_normal.to_series()
df = pd.DataFrame({
    # "peaks": so2_peaks_ser,
    "close": so2_peaks_close_ser,
    "big": so2_peaks_big_ser,
    "small": so2_peaks_small_ser,
    "normal": peaks_normal_ser,
})
print(
